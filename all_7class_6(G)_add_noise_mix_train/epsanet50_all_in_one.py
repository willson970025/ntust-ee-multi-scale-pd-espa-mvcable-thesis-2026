...
# 訓練集：擴增（1 張變 10 張，每個 epoch 隨機位移）
...
# 驗證集：擴增（1 張變 10 張，每個 epoch 隨機位移）
val_ds = tf.data.Dataset.from_tensor_slices((val_paths, val_labels))
val_ds = val_ds.map(load_image, num_parallel_calls=tf.data.AUTOTUNE)
val_ds = val_ds.cache()  # [優化] 快取載入後的圖片
val_ds = val_ds.map(augment_with_circular_shift, num_parallel_calls=tf.data.AUTOTUNE)
...
# [新增] 測試集（擴增版）：與訓練/驗證集相同的擴增方式，用於比較
...
augmented_val_size = val_size * AUGMENT_FACTOR
...
print(f"  擴增後驗證集: {augmented_val_size} 張 (原始 {val_size} 張)")
print(f"  測試集（原始）: {test_size} 張 (不擴增)")
print(f"  測試集（擴增）: {augmented_test_size} 張 (用於比較)")
...
plt.title(f'原始測試集混淆矩陣 (Acc: {test_acc_original:.4f})', fontsize=15, fontweight='bold')
...
plt.xlabel('Predicted Label', fontsize=13)
plt.ylabel('True Label', fontsize=13)
plt.title(f'擴增測試集混淆矩陣 (Acc: {test_acc_augmented:.4f})', fontsize=15, fontweight='bold')
...
with open(log_path, 'w', encoding='utf-8') as f:
    ...
    f.write(f"輸入: {INPUT_WIDTH}x{INPUT_HEIGHT}x3 (多通道: 原圖+SobelX+SobelY)\n")
    ...
    f.write(f"擴增後: Train={augmented_train_size}, Val={augmented_val_size}\n\n")
    ...
    # ========== 擴增測試集 ==========
    f.write("【擴增測試集】\n")
    f.write(f"Accuracy: {test_acc_augmented:.4f} (原始vs擴增: {(test_acc_augmented - test_acc_original)*100:+.2f}%)\n")
...
print(f"  原始測試集準確率: {test_acc_original:.4f} ({test_acc_original*100:.2f}%)")
print(f"  擴增測試集準確率: {test_acc_augmented:.4f} ({test_acc_augmented*100:.2f}%)")
print(f"  訓練輪數: {actual_epochs}")
print(f"  訓練時間: {str(timedelta(seconds=int(training_duration)))}")
