# -*- coding: utf-8 -*-
"""
批次預測與自動分類程式 (EPSANet-Large 版本 + EigenCAM/Grad-CAM)
...
特點：
  ...
  - 為每張圖片生成 EigenCAM/Grad-CAM 熱力圖，顯示模型關注的區域
  - 效能優化：XLA 編譯

CAM 方法說明：
  - EigenCAM: 使用 SVD 取第一主成分，class-agnostic，速度極快
  - Grad-CAM: 使用類別梯度加權，class-specific，熱力圖更有區分度
"""
...
INPUT_ROOT_DIRS = [
    ...
    r'/home/cckuo/m11307u09/GG/1126/data_GG_discharge_filter/inbetween_discharge'
]

# 輸出根目錄的基礎路徑
OUTPUT_BASE_DIR = r'/home/cckuo/m11307u09/GG/1126/epsanet50/all in one/all_7class_6(G)_add_noise_mix_train/GG_gradcam_1'
...
# CAM 方法選擇
# 'eigencam' = EigenCAM（SVD 第一主成分，class-agnostic，速度快但熱力圖可能相似）
# 'gradcam' = Grad-CAM（使用類別梯度，class-specific，熱力圖更有區分度）
CAM_METHOD = 'gradcam'

print("="*80)
print("批次預測與自動分類程式 (EPSANet-Large 版本 + EigenCAM)")
print("="*80)
...
print(f"效能優化: XLA 編譯")
print(f"CAM 方法: {CAM_METHOD.upper()}")
if CAM_METHOD == 'gradcam':
    print(f"  Grad-CAM: 使用類別梯度加權，class-specific，熱力圖更有區分度")
else:
    print(f"  EigenCAM: 使用 SVD 取第一主成分，class-agnostic，速度極快")
print("="*80)
...
# ============================================================================
# EigenCAM 相關函數（EPSANet-Large 版本）
# ============================================================================

# 快取 EigenCAM 子模型
_eigencam_activation_model = None
_eigencam_target_layer = None

# 快取 Grad-CAM 子模型和函數
_gradcam_model = None
_gradcam_target_layer = None
_gradcam_compute_fn = None
...
def list_available_layers():
    """
    列出模型中所有可用於 EigenCAM 的層（有 4D 輸出的卷積層）
    """
    print("\n  【可用的目標層列表】")
    print("  " + "-"*70)
...
EIGENCAM_TARGET_LAYER = 'layer1'


def get_epsanet_target_layer():
    """
    獲取 EPSANet-Large 的目標層用於 EigenCAM
    ...
    if EIGENCAM_TARGET_LAYER == 'auto':
    ...
    elif EIGENCAM_TARGET_LAYER == 'gradcam_target':
    ...
    elif EIGENCAM_TARGET_LAYER.startswith('layer'):
        # 使用指定的 layer 級別（如 'layer3', 'layer2'）
        layer_prefix = EIGENCAM_TARGET_LAYER
        candidates = []
        for info in available_layers:
        ...
        # 直接使用指定的層名稱
        for layer in model.layers:
            if layer.name == EIGENCAM_TARGET_LAYER:
                target_layer = layer
                target_layer_name = layer.name
                break
    ...
    if target_layer is None:
        print(f"  [!] 指定的層 '{EIGENCAM_TARGET_LAYER}' 未找到，嘗試回退...")
    ...
    if target_layer is None:
        raise RuntimeError("找不到任何適合的目標層，無法生成 EigenCAM")
...
def build_eigencam_model(target_layer):
    """
    建立 EigenCAM 子模型（快取以提升效能）
    返回 activation_model: 輸出目標層的特徵圖
    """
    global _eigencam_activation_model, _eigencam_target_layer

    if _eigencam_activation_model is not None and _eigencam_target_layer == target_layer:
        return _eigencam_activation_model

    try:
        _eigencam_activation_model = Model(
            inputs=model.input,
            outputs=target_layer.output
        )
        _eigencam_target_layer = target_layer
        return _eigencam_activation_model
    except Exception as e:
        raise RuntimeError(f"建立 EigenCAM 子模型失敗: {e}")


def make_gradcam_heatmaps_batch(img_batch, target_layer, pred_classes, verbose_diag=False):
    """
    批次 Grad-CAM 熱力圖生成（Class-specific）- 優化版：快取模型和函數
    """
    global _gradcam_model, _gradcam_target_layer, _gradcam_compute_fn

    if target_layer is None:
        raise RuntimeError("Grad-CAM 目標層為 None")

    n_samples = img_batch.shape[0]
    diag_info = {} if verbose_diag else None

    try:
        # 快取 Grad-CAM 模型（只建立一次）
        if _gradcam_model is None or _gradcam_target_layer != target_layer:
            print("  [Grad-CAM] 建立模型（只執行一次）...")
            _gradcam_model = Model(
                inputs=model.input,
                outputs=[target_layer.output, model.output]
            )
            _gradcam_target_layer = target_layer

            # 建立快取的 tf.function（只 trace 一次）
            @tf.function(reduce_retracing=True)
            def _compute_gradcam(images, class_indices):
                with tf.GradientTape() as tape:
                    conv_outputs, predictions = _gradcam_model(images, training=False)
                    batch_indices = tf.range(tf.shape(class_indices)[0])
            ...
            _gradcam_compute_fn = _compute_gradcam
            print("  [Grad-CAM] 模型建立完成")

        img_tensor = tf.cast(img_batch, tf.float32)
        pred_classes_tensor = tf.cast(pred_classes, tf.int32)

        # 使用快取的函數
        conv_outputs, grads = _gradcam_compute_fn(img_tensor, pred_classes_tensor)
        ...
        if verbose_diag:
            ...
            conv_np = conv_outputs.numpy()

            diag_info['conv_output_shape'] = conv_np.shape
            diag_info['gradients_shape'] = grads_np.shape
            diag_info['weights_shape'] = weights_np.shape
            diag_info['gradients_mean'] = float(np.mean(np.abs(grads_np)))
            diag_info['gradients_std'] = float(np.std(grads_np))
            diag_info['weights_mean'] = float(np.mean(np.abs(weights_np)))

            # 檢查不同圖片的權重差異
            if n_samples >= 2:
                weight_correlations = []
                for i in range(min(5, n_samples - 1)):
                    corr = np.corrcoef(weights_np[i], weights_np[i + 1])[0, 1]
                    weight_correlations.append(float(corr))
                diag_info['weight_correlations'] = weight_correlations
                diag_info['avg_weight_correlation'] = float(np.mean(weight_correlations))

        # 加權求和：對每個空間位置，用權重加權通道
        # conv_outputs: (N, H, W, C), weights: (N, C)
        # 擴展 weights 維度以便廣播
        weights_expanded = tf.reshape(weights, [-1, 1, 1, tf.shape(weights)[1]])  # (N, 1, 1, C)
        cam = tf.reduce_sum(conv_outputs * weights_expanded, axis=-1)  # (N, H, W)

        # ReLU：只保留正向影響
        cam = tf.nn.relu(cam)

        # 上採樣到輸入尺寸
        cam_4d = tf.expand_dims(cam, axis=-1)  # (N, H, W, 1)
        cam_resized = tf.image.resize(cam_4d, [INPUT_HEIGHT, INPUT_WIDTH], method='bilinear')
        heatmaps = tf.squeeze(cam_resized, axis=-1)  # (N, INPUT_HEIGHT, INPUT_WIDTH)

        # 每張圖獨立正規化到 [0, 1]
        heatmaps_flat = tf.reshape(heatmaps, [n_samples, -1])
        heatmap_min = tf.reduce_min(heatmaps_flat, axis=1, keepdims=True)
        heatmap_max = tf.reduce_max(heatmaps_flat, axis=1, keepdims=True)

        heatmap_range = heatmap_max - heatmap_min
        heatmap_range = tf.where(heatmap_range > 0, heatmap_range, tf.ones_like(heatmap_range))

        heatmaps_normalized = (heatmaps_flat - heatmap_min) / heatmap_range
        heatmaps = tf.reshape(heatmaps_normalized, [n_samples, INPUT_HEIGHT, INPUT_WIDTH])

        if verbose_diag:
            hm_final = heatmaps.numpy()

            # 檢查不同圖片的熱力圖相似度
            if n_samples >= 2:
                hm_correlations = []
                for i in range(min(5, n_samples - 1)):
                    corr = np.corrcoef(hm_final[i].flatten(), hm_final[i + 1].flatten())[0, 1]
                    hm_correlations.append(float(corr))
                diag_info['heatmap_correlations'] = hm_correlations
                diag_info['avg_heatmap_correlation'] = float(np.mean(hm_correlations))

            # 空間分布分析
            final_spatial = np.mean(hm_final, axis=0)
            h_final, w_final = final_spatial.shape
            center_h_s, center_h_e = h_final // 4, 3 * h_final // 4
            center_w_s, center_w_e = w_final // 4, 3 * w_final // 4

            center_mask = np.zeros_like(final_spatial, dtype=bool)
            center_mask[center_h_s:center_h_e, center_w_s:center_w_e] = True

            diag_info['final_center_mean'] = float(np.mean(final_spatial[center_mask]))
            diag_info['final_edge_mean'] = float(np.mean(final_spatial[~center_mask]))
            diag_info['center_to_edge_ratio'] = float(
                diag_info['final_center_mean'] / (diag_info['final_edge_mean'] + 1e-8)
            )

        if verbose_diag:
            return heatmaps.numpy(), diag_info
        return heatmaps.numpy()

    except Exception as e:
        import traceback
        print(f"  Grad-CAM 計算失敗: {e}")
        print(f"  詳細錯誤:\n{traceback.format_exc()}")
        if verbose_diag:
            return np.zeros((n_samples, INPUT_HEIGHT, INPUT_WIDTH), dtype=np.float32), {'error': str(e)}
        return np.zeros((n_samples, INPUT_HEIGHT, INPUT_WIDTH), dtype=np.float32)


def make_eigencam_heatmaps_batch(img_batch, target_layer, verbose_diag=False):
    """
    完全向量化的批次 EigenCAM 熱力圖生成

    EigenCAM 演算法（向量化版本）：
    1. 一次前向傳播獲取所有特徵圖 (N, H, W, C)
    2. Reshape 為 (N, H*W, C)
    3. 批次 SVD 計算，取第一主成分
    4. Reshape 回 (N, H, W) 並上採樣到輸入尺寸
    5. 批次正規化到 [0, 1]

    參數:
        img_batch: 預處理後的圖片批次 (N, H, W, 3)
        target_layer: EPSANet 的目標層
        verbose_diag: 是否輸出詳細診斷資訊

    返回:
        heatmaps: 熱力圖 numpy array (N, H, W)
        diag_info: 診斷資訊字典（僅當 verbose_diag=True 時返回）
    """
    if target_layer is None:
        raise RuntimeError("EigenCAM 目標層為 None")

    activation_model = build_eigencam_model(target_layer)
    n_samples = img_batch.shape[0]
    diag_info = {} if verbose_diag else None
    ...
    try:
        # 一次前向傳播獲取所有特徵圖
        img_tensor = tf.cast(img_batch, tf.float32)
        activations = activation_model(img_tensor, training=False)  # (N, h, w, C)
        ...
        if verbose_diag:
            ...
            diag_info['activation_shape'] = act_np.shape
            diag_info['activation_mean'] = float(np.mean(act_np))
            diag_info['activation_std'] = float(np.std(act_np))
            diag_info['activation_min'] = float(np.min(act_np))
            diag_info['activation_max'] = float(np.max(act_np))

            # 分析特徵圖的空間分布（檢查是否本身就集中在中間）
            # 計算每個空間位置的平均激活強度
            spatial_activation = np.mean(act_np, axis=(0, 3))  # (h, w)
            diag_info['spatial_activation_shape'] = spatial_activation.shape

            # 計算中心區域 vs 邊緣區域的激活比例
            h_np, w_np = spatial_activation.shape
            center_h_start, center_h_end = h_np // 4, 3 * h_np // 4
            center_w_start, center_w_end = w_np // 4, 3 * w_np // 4
            center_region = spatial_activation[center_h_start:center_h_end, center_w_start:center_w_end]
            edge_mask = np.ones_like(spatial_activation, dtype=bool)
            edge_mask[center_h_start:center_h_end, center_w_start:center_w_end] = False
            edge_region = spatial_activation[edge_mask]

            diag_info['center_activation_mean'] = float(np.mean(center_region))
            diag_info['edge_activation_mean'] = float(np.mean(edge_region))
            diag_info['center_to_edge_ratio'] = float(np.mean(center_region) / (np.mean(edge_region) + 1e-8))

            # 檢查不同圖片之間激活的差異
            per_image_means = np.mean(act_np, axis=(1, 2, 3))
            diag_info['per_image_activation_means'] = per_image_means[:10].tolist()  # 只取前10張
            diag_info['per_image_activation_std'] = float(np.std(per_image_means))

        # Reshape 為 (N, H*W, C) 以進行批次 SVD
        activations_2d = tf.reshape(activations, [n, h * w, c])

        # 批次 SVD 分解
        # tf.linalg.svd 支援批次操作，輸入 (N, M, K) 會對每個 (M, K) 矩陣計算 SVD
        s, u, v = tf.linalg.svd(activations_2d, full_matrices=False)
        # s: (N, min(H*W, C)) - 奇異值
        # u: (N, H*W, min(H*W, C)) - 左奇異向量
        # v: (N, C, min(H*W, C)) - 右奇異向量
        ...
        if verbose_diag:
            ...
            # 奇異值分析
            diag_info['singular_values_shape'] = s_np.shape
            diag_info['top_5_singular_values_sample0'] = s_np[0, :5].tolist()
            diag_info['singular_value_ratio_s0_s1'] = float(s_np[0, 0] / (s_np[0, 1] + 1e-8))

            # 檢查第一主成分是否在不同圖片間相似（問題的關鍵！）
            u_first = u_np[:, :, 0]  # (N, H*W)

            # 計算不同圖片的第一主成分之間的相關係數
            if n_samples >= 2:
                correlations = []
                for i in range(min(5, n_samples - 1)):
                    corr = np.corrcoef(u_first[i], u_first[i + 1])[0, 1]
                    correlations.append(float(corr))
                diag_info['first_component_correlations'] = correlations
                diag_info['avg_first_component_correlation'] = float(np.mean(correlations))

            # 檢查第一主成分的空間分布
            u_first_reshaped = u_first[0].reshape(h.numpy(), w.numpy())
            diag_info['first_component_center_mean'] = float(np.mean(np.abs(u_first_reshaped[center_h_start:center_h_end, center_w_start:center_w_end])))
            diag_info['first_component_edge_mean'] = float(np.mean(np.abs(u_first_reshaped[edge_mask.reshape(h.numpy(), w.numpy())])))

        # 取第一主成分：u[:, :, 0] * s[:, 0:1]
        # s[:, 0] 是最大奇異值，u[:, :, 0] 是對應的左奇異向量
        first_component = u[:, :, 0] * tf.expand_dims(s[:, 0], axis=1)  # (N, H*W)

        # Reshape 回 (N, H, W)
        heatmaps = tf.reshape(first_component, [n, h, w])

        # 取絕對值（主成分可能是負的）
        heatmaps = tf.abs(heatmaps)

        if verbose_diag:
            hm_before_resize = heatmaps.numpy()
            diag_info['heatmap_before_resize_shape'] = hm_before_resize.shape
            diag_info['heatmap_before_resize_mean'] = float(np.mean(hm_before_resize))
            diag_info['heatmap_before_resize_std'] = float(np.std(hm_before_resize))

            # 檢查不同圖片的熱力圖相似度
            if n_samples >= 2:
                hm_correlations = []
                for i in range(min(5, n_samples - 1)):
                    corr = np.corrcoef(hm_before_resize[i].flatten(), hm_before_resize[i + 1].flatten())[0, 1]
                    hm_correlations.append(float(corr))
                diag_info['heatmap_correlations_before_norm'] = hm_correlations
                diag_info['avg_heatmap_correlation_before_norm'] = float(np.mean(hm_correlations))

        # 批次上採樣到輸入尺寸 (N, INPUT_HEIGHT, INPUT_WIDTH)
        heatmaps_4d = tf.expand_dims(heatmaps, axis=-1)  # (N, h, w, 1)
        heatmaps_resized = tf.image.resize(heatmaps_4d, [INPUT_HEIGHT, INPUT_WIDTH], method='bilinear')
        heatmaps = tf.squeeze(heatmaps_resized, axis=-1)  # (N, INPUT_HEIGHT, INPUT_WIDTH)

        # 批次正規化到 [0, 1]（每張圖獨立正規化）
        heatmaps_flat = tf.reshape(heatmaps, [n, -1])  # (N, H*W)
        heatmap_min = tf.reduce_min(heatmaps_flat, axis=1, keepdims=True)  # (N, 1)
        heatmap_max = tf.reduce_max(heatmaps_flat, axis=1, keepdims=True)  # (N, 1)

        if verbose_diag:
            diag_info['heatmap_min_values'] = heatmap_min.numpy()[:10, 0].tolist()
            diag_info['heatmap_max_values'] = heatmap_max.numpy()[:10, 0].tolist()
            diag_info['heatmap_dynamic_range'] = (heatmap_max.numpy()[:10, 0] - heatmap_min.numpy()[:10, 0]).tolist()
        ...
        heatmaps_normalized = (heatmaps_flat - heatmap_min) / heatmap_range
        heatmaps = tf.reshape(heatmaps_normalized, [n, INPUT_HEIGHT, INPUT_WIDTH])

        if verbose_diag:
            hm_final = heatmaps.numpy()

            # 最終熱力圖的空間分布分析
            final_spatial = np.mean(hm_final, axis=0)  # (H, W)
            h_final, w_final = final_spatial.shape
            center_h_s, center_h_e = h_final // 4, 3 * h_final // 4
            center_w_s, center_w_e = w_final // 4, 3 * w_final // 4

            diag_info['final_heatmap_center_mean'] = float(np.mean(final_spatial[center_h_s:center_h_e, center_w_s:center_w_e]))
            edge_mask_final = np.ones_like(final_spatial, dtype=bool)
            edge_mask_final[center_h_s:center_h_e, center_w_s:center_w_e] = False
            diag_info['final_heatmap_edge_mean'] = float(np.mean(final_spatial[edge_mask_final]))
            diag_info['final_center_to_edge_ratio'] = float(
                diag_info['final_heatmap_center_mean'] / (diag_info['final_heatmap_edge_mean'] + 1e-8)
            )
            ...
            if n_samples >= 2:
                ...
                for i in range(min(5, n_samples - 1)):
                    corr = np.corrcoef(hm_final[i].flatten(), hm_final[i + 1].flatten())[0, 1]
                    final_correlations.append(float(corr))
                diag_info['final_heatmap_correlations'] = final_correlations
                diag_info['avg_final_heatmap_correlation'] = float(np.mean(final_correlations))
...
def apply_eigencam_to_image(img_array, heatmap, alpha=0.4, colormap=cv2.COLORMAP_JET):
    """
    將 EigenCAM 熱力圖疊加到原圖上

    參數:
        img_array: 圖片 (H, W, 3)，0-255 範圍
        heatmap: EigenCAM 熱力圖 (H', W')
        alpha: 熱力圖透明度 (0-1)
        colormap: OpenCV色彩映射
...
if epsanet_target_layer is not None:
    print(f" EigenCAM 初始化成功！使用層: {epsanet_target_layer.name}")
    _ = build_eigencam_model(epsanet_target_layer)
else:
    print(" EigenCAM 初始化失敗")
    exit(1)

# Warm-up: 預先觸發 Grad-CAM 的 tf.function tracing
print("\n 進行 Grad-CAM warm-up（觸發 tf.function tracing）...")
_warmup_img = np.zeros((1, INPUT_HEIGHT, INPUT_WIDTH, 3), dtype=np.float32)
_warmup_classes = np.array([0], dtype=np.int32)
_ = make_gradcam_heatmaps_batch(_warmup_img, epsanet_target_layer, _warmup_classes)
print(" Grad-CAM warm-up 完成！")
...
# ============================================================================
# 圖片預處理函數（EPSANet-Large 版本）- 原始 CPU 版本，保留供參考
# ============================================================================
def preprocess_image(image_path):
    """
    載入並預處理單張圖片（與 epsanet50_all_in_one.py 訓練時一致）

    預處理流程：
    1. 讀取 RGB 圖片
    2. 二值化（訓練資料已是二值化圖片，預測時需將彩色圖片二值化）
    3. 黑白翻轉 (255 - gray)
    4. Resize 到模型輸入尺寸
    5. 計算 Sobel X/Y 邊緣（使用與訓練相同的 kernel）
    6. 堆疊為 3 通道
    7. ResNet 預處理
    """
    try:
        ...
        # 二值化：純白(255,255,255)以外的顏色轉黑(0)
        # 訓練資料已是二值化圖片，預測時需將彩色圖片做相同處理
        white_mask = np.all(img_array > 254, axis=-1)
        gray = np.where(white_mask, 255.0, 0.0).astype(np.float32)

        # 黑白翻轉（與訓練一致）
        gray = 255.0 - gray

        # 調整尺寸到模型輸入大小
        gray_resized = cv2.resize(gray, (INPUT_WIDTH, INPUT_HEIGHT), interpolation=cv2.INTER_LINEAR)

        if USE_MULTICHANNEL:
            channel_1 = gray_resized

            # Sobel X（使用與訓練相同的 kernel）
            # 訓練時 kernel: [[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]
            sobel_x_kernel = np.array([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=np.float32)
            sobel_x = cv2.filter2D(gray_resized, cv2.CV_32F, sobel_x_kernel)
            sobel_x = np.abs(sobel_x)
            sobel_x_max = np.max(sobel_x)
            if sobel_x_max > 0:
                sobel_x = sobel_x / sobel_x_max * 255.0
            channel_2 = sobel_x

            # Sobel Y（使用與訓練相同的 kernel）
            # 訓練時 kernel: [[-1, -2, -1], [0, 0, 0], [1, 2, 1]]
            sobel_y_kernel = np.array([[-1, -2, -1], [0, 0, 0], [1, 2, 1]], dtype=np.float32)
            sobel_y = cv2.filter2D(gray_resized, cv2.CV_32F, sobel_y_kernel)
            sobel_y = np.abs(sobel_y)
            sobel_y_max = np.max(sobel_y)
            if sobel_y_max > 0:
                sobel_y = sobel_y / sobel_y_max * 255.0
            channel_3 = sobel_y

            img_multichannel = np.stack([channel_1, channel_2, channel_3], axis=-1)
        else:
            img_multichannel = np.stack([gray_resized, gray_resized, gray_resized], axis=-1)

        img_preprocessed = preprocess_input(img_multichannel)
        return img_preprocessed

    except Exception as e:
        print(f"  處理圖片失敗 {image_path}: {e}")
        return None
...
# 批次預測函數（原始版本，保留供參考）
# ============================================================================
def predict_batch(image_paths, model, batch_size=64):
    """批次預測圖片（原始串行版本）"""
    images = []
    valid_indices = []

    print(f"  正在載入 {len(image_paths)} 張圖片（輸入尺寸: {INPUT_WIDTH}x{INPUT_HEIGHT}）...")
    for idx, img_path in enumerate(tqdm(image_paths, desc="  載入圖片")):
    ...
    print(f"  正在預測 {len(images)} 張圖片（Batch Size: {batch_size}）...")
    predictions = model.predict(images, batch_size=batch_size, verbose=0)

    return predictions, images, valid_indices
...
# 批次預測（流水線優化版）
# ============================================================================
def predict_batch_optimized(image_paths, model, original_cache=None, batch_size=512):
    """
    批次預測圖片（使用 tf.data.Dataset 流水線）

    優化重點：
    - 使用 GPU 加速的 tf_preprocess_image 進行預處理
    - 透過 prefetch 實現 CPU/GPU 流水線並行
    - 原圖載入在背景執行緒執行

    Args:
        image_paths: 圖片路徑列表
        model: 模型（目前未使用，保留介面相容性）
        original_cache: 原圖快取物件
        batch_size: 批次大小

    Returns:
        (None, Dataset, valid_indices): Dataset 供後續推論使用
    """
    import time

    n_images = len(image_paths)
    print(f"  [階段1] 建立 Dataset 流水線（{n_images} 張圖片）...")
    ...
    # 建立 Dataset 流水線（GPU 預處理 + prefetch）
    dataset = create_inference_dataset(image_paths, batch_size)

    # 收集預處理結果
    batches = []
    for batch in tqdm(dataset, desc="  GPU預處理", total=(n_images + batch_size - 1) // batch_size):
        batches.append(batch.numpy())

    print(f"  [階段1] 預處理完成: {time.time() - load_start:.2f} 秒")
    ...
    print(f"  [階段2] 合併批次: {time.time() - convert_start:.2f} 秒")
    ...
    if cache_thread is not None:
        ...
        cache_thread.join()
        print(f"  [階段3] 原圖快取完成: {time.time() - wait_start:.2f} 秒")

    # 所有圖片都視為有效（Dataset 流水線會跳過錯誤）
    valid_indices = list(range(n_images))

    return None, preprocessed_images, valid_indices
...
def save_single_result(args):
    ...
    (img_path, pred_class_name, pred_class_idx, heatmap, pred_prob, all_class_probs,
     subfolder_output_dir, original_cache, cam_method) = args
    ...
    try:
        ...
        # 保存 CAM 圖
        cam_img = apply_eigencam_to_image(img_to_save, heatmap, alpha=0.4)
        cam_suffix = '_gradcam.png' if cam_method == 'gradcam' else '_eigencam.png'
        cam_output_path = os.path.join(class_output_dir, final_base_name + cam_suffix)
...
def process_all_subfolders():
    ...
    for root_idx, INPUT_ROOT_DIR in enumerate(INPUT_ROOT_DIRS, 1):
        ...
        for subfolder_idx, subfolder_name in enumerate(subfolders, 1):
            ...
            if CAM_METHOD == 'gradcam':
                ...
                for batch in tqdm(dataset, desc="  預處理", total=(n_images + BATCH_SIZE - 1) // BATCH_SIZE):
                    batches.append(batch.numpy())
                preprocessed_images = np.concatenate(batches, axis=0)
                print(f"  [階段A] 預處理完成: {time.time() - preprocess_start:.2f} 秒")
                ...
                print(f"  [階段B] 推論+GradCAM 完成: {time.time() - inference_start:.2f} 秒")
                ...
                preprocess_start = time.time()
                dataset = create_inference_dataset(image_paths, BATCH_SIZE)
                batches = [batch.numpy() for batch in tqdm(dataset, desc="  GPU預處理",
                           total=(n_images + BATCH_SIZE - 1) // BATCH_SIZE)]
                preprocessed_images = np.concatenate(batches, axis=0)
                print(f"  [預處理] 完成: {time.time() - preprocess_start:.2f} 秒")

                print(f"\n  正在執行推論...")
                predictions = model.predict(preprocessed_images, batch_size=BATCH_SIZE, verbose=0)
                pred_classes = np.argmax(predictions, axis=1)
                pred_probs = np.max(predictions, axis=1)

                print(f"  正在計算 EigenCAM...")
                heatmaps = make_eigencam_heatmaps_batch(
                    preprocessed_images, epsanet_target_layer, verbose_diag=False
                )
            ...
            for i in range(len(valid_indices)):
                ...
                task = (img_path, pred_class_name, pred_class_idx, heatmap, pred_prob,
                        all_class_probs, subfolder_output_dir, original_cache, CAM_METHOD)
                save_tasks.append(task)

            # 執行並行保存
            save_results = save_results_parallel(save_tasks, max_workers=12)
            save_time = time.time() - save_start
            print(f"  並行保存完成，耗時: {save_time:.2f} 秒")
        ...
        if len(root_results) > 0:
            ...
            csv_path = os.path.join(OUTPUT_ROOT_DIR, f'prediction_report_eigencam_{root_start_time.strftime("%Y%m%d_%H%M%S")}.csv')
