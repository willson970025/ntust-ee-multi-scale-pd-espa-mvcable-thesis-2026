...
# ============================================================================
# ResNet50 模型定義 (Baseline，含 Weight Decay)
# ============================================================================

def build_resnet50(input_shape=(80, 180, 3), num_classes=7, dropout_rate=0.2,
                   weight_decay=WEIGHT_DECAY):
    """
    建立 ResNet50 模型（PRPD 七類別分類 baseline）

    - 從頭訓練，不使用 ImageNet 預訓練權重
    - 使用 Bottleneck block，block 數 [3, 4, 6, 3]
    - 各 stage bottleneck planes [64, 128, 256, 512]，輸出通道 [256, 512, 1024, 2048]
    ...
    def bottleneck_block(x, planes, stride, name):
        identity = x

        out = layers.Conv2D(planes, 1, strides=1, use_bias=False,
        ...
        out = layers.Conv2D(planes * expansion, 1, strides=1, use_bias=False,
                            kernel_regularizer=reg, name=f'{name}_conv3')(out)
        ...
        if stride != 1 or in_channels != planes * expansion:
            identity = layers.Conv2D(planes * expansion, 1, strides=stride, use_bias=False,
        ...
        out = layers.ReLU(name=f'{name}_relu3')(out)
        return out

    blocks = [3, 4, 6, 3]
    planes_cfg = [64, 128, 256, 512]
    strides = [1, 2, 2, 2]
    ...
    for stage_idx in range(4):
        for block_idx in range(blocks[stage_idx]):
            ...
            x = bottleneck_block(x, planes_cfg[stage_idx], stride,
    ...
    model = Model(inputs=inputs, outputs=outputs, name='ResNet50_PRPD')
...
output_dir = '/home/cckuo/m11307u09/GG/1126/epsanet50/all in one/model_compare_resnet50'

print("=" * 80)
print(f"ResNet50 7類別模型訓練 - 輸入尺寸: {INPUT_WIDTH}x{INPUT_HEIGHT} - 含 Dropout + Weight Decay")
print("=" * 80)
...
print(f"\n模型架構 (ResNet50 Baseline + Dropout + Weight Decay + 多通道特徵增強):")
print(f"  輸入尺寸: {INPUT_SHAPE} (HxWxC)")
print(f"  Base Model: ResNet50 (Bottleneck, blocks=[3, 4, 6, 3])")
print(f"  通道配置: [256, 512, 1024, 2048]")
print(f"  架構: ResNet50 -> GAP -> Dropout({DROPOUT_RATE}) -> Dense(1024) -> BN -> ReLU")
...
# 回調函數
# ============================================================================
model_path = os.path.join(output_dir, 'best_model_resnet50_dropout_wd_180x80.keras')
...
# 訓練
print(f"\n{'='*80}")
print(f"開始訓練 ResNet50 (含 Dropout + Weight Decay + 多通道特徵增強)...")
print(f"{'='*80}\n")
...
plt.title(f'ResNet50 Loss Curve (Input: {INPUT_WIDTH}x{INPUT_HEIGHT}, WD={WEIGHT_DECAY})', fontsize=14, fontweight='bold')
...
plt.title(f'ResNet50 Accuracy Curve (Input: {INPUT_WIDTH}x{INPUT_HEIGHT}, WD={WEIGHT_DECAY})', fontsize=14, fontweight='bold')
plt.legend(fontsize=11)
plt.grid(True, alpha=0.3)
...
plt.title('ResNet50 Precision & Recall Curve', fontsize=14, fontweight='bold')
...
plt.title('ResNet50 Overfitting Analysis', fontsize=14, fontweight='bold')
plt.legend(fontsize=11)
plt.grid(True, alpha=0.3)
plt.axhline(y=0, color='black', linestyle='--', alpha=0.5)

plt.tight_layout()
save_path = os.path.join(output_dir, 'training_curves_resnet50_dropout_wd_180x80.png')  # [修改] 檔名加上 _wd
...
with open(log_path, 'w', encoding='utf-8') as f:
    ...
    f.write(f"通道: [256, 512, 1024, 2048] -> GAP(2048) -> FC(1024) -> FC(512) -> {num_classes}類\n")
    ...
    f.write("本模型作為第 4.5 節不同模型辨識效能比較之 baseline。為確保比較公平性，"
            ...
            "未擴增驗證集、最佳權重選取方式與原始測試集評估流程，僅將模型骨幹替換為 ResNet50。\n\n")
...
print("訓練摘要")
print("="*80)
print(f"  模型: ResNet50 (含 Weight Decay)")
