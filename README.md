...
### 消融實驗（第 4.6 節）
```bash
cd "all_7class_6(G)_add_noise_mix_train/ablation_results"
python epsanet50_ablation_all_in_one.py
```
依序執行注意力消融、輸入通道消融、雜訊穩健性三組共 6 個變體；產出之 `ablation_summary.csv` 與 `ablation_summary_for_thesis.md` 即為論文第 4.6 節表 4-5 / 4-6 / 4-7 數據來源。
