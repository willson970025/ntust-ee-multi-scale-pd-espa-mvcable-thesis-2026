...
- 驗證/評估一律使用「未擴增驗證折」(augment=False)，與論文報告一致
- 每折輸出模型、混淆矩陣、訓練曲線、完整分類報告 (classification_report_foldX.csv)
- t-SNE 使用各 fold 的未擴增驗證折
- 彙整 kfold_summary.csv、fold_class_distribution.csv、整體 OOF 結果
- 五個 fold 模型分別評估固定測試集 (test_metrics_by_fold.csv) 並計算平均±標準差
- 輸出 environment_info.* 與 experiment_config.json 供論文表 4-1 與可重現性
- 最終彙整平均準確率與標準差
"""
# ============================================================================
...
output_dir = '/home/cckuo/m11307u09/GG/1126/epsanet50/all in one/all_7class_7(G)_add_noise_mix_train_kfold'
...
def create_dataset(paths, labels, augment=True, shuffle=True):
    ...
    ds = ds.cache()  # 快取「已載入的圖」(load_image 為確定性)，驗證折也只解碼一次
...
# 測試集（擴增版）
# [修正] 加入 cache，使擴增測試集只生成一次並固定，五個 fold 模型評估時共用同一份資料
...
test_ds_augmented = test_ds_augmented.cache()  # 固定擴增測試資料（穩健性觀察用）
...
# t-SNE 特徵視覺化函數
# ============================================================================
def generate_tsne_visualization(model, eval_ds, eval_labels, output_dir, eval_size, title_prefix=""):
    """
    從模型的三個層提取特徵並生成 t-SNE 視覺化圖

    特徵提取層：
    - global_avg_pool: GAP 層輸出 (4096 維)  [論文主文主要使用]
    - head_fc1: 第一個全連接層輸出 (1024 維)
    - head_fc2: 第二個全連接層輸出 (512 維)

    Args:
        model: 訓練好的 Keras 模型
        eval_ds: 評估用 tf.data.Dataset（K-Fold 時為該 fold 的未擴增驗證折）
        eval_labels: 評估標籤 (numpy array)
        output_dir: 輸出目錄
        eval_size: 評估樣本數
    ...
    for layer_name, tsne_data in tsne_results.items():
        ...
        plt.title(f'{title_prefix}驗證集 t-SNE 特徵視覺化 - {layer_name}\n(perplexity={perplexity}, 驗證集 {eval_size} 張)',
    ...
    fig.suptitle(f'{title_prefix}驗證集 t-SNE 特徵視覺化比較 (驗證集 {eval_size} 張, perplexity={perplexity})',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
...
# ============================================================================
# 訓練超參數（提前定義，供 experiment_config.json 與訓練迴圈共用）
# ============================================================================
target_lr = 0.002
min_lr = 1e-5
OPTIMIZER_NAME = 'SGD'
...
# ============================================================================
# 實驗環境輸出 (environment_info.txt / environment_info.json) - 供論文表 4-1
# ============================================================================
def collect_environment_info():
    """蒐集作業系統、Python/TF/Keras 版本與 GPU 資訊"""
    info = {}
    ...
    # 透過 nvidia-smi 取得更完整的 GPU 名稱 / 記憶體 / driver / CUDA 版本
    driver_version = None
    cuda_version = None
    try:
        ...
        for line in smi_out.splitlines():
            ...
            if len(parts) >= 3:
                smi_names.append(parts[0])
                gpu_memories.append(parts[1])
    ...
    # CUDA 版本（nvidia-smi 頂端資訊）
    try:
        smi_full = subprocess.check_output(
            ['nvidia-smi'], encoding='utf-8', stderr=subprocess.DEVNULL
        )
        if 'CUDA Version:' in smi_full:
            cuda_version = smi_full.split('CUDA Version:')[1].strip().split()[0]
    except Exception:
        pass

    # TensorFlow 內建的 CUDA / cuDNN 編譯版本
    try:
        build_info = tf.sysconfig.get_build_info()
        if cuda_version is None:
            cuda_version = build_info.get('cuda_version')
        info['cudnn_version'] = str(build_info.get('cudnn_version', 'unknown'))
    except Exception:
        info['cudnn_version'] = 'unknown'

    info['gpu_name'] = ', '.join(gpu_names) if gpu_names else 'N/A (CPU only)'
    info['gpu_memory'] = ', '.join(gpu_memories) if gpu_memories else 'unknown'
    info['driver_version'] = driver_version if driver_version else 'unknown'
    info['cuda_version'] = cuda_version if cuda_version else 'unknown'
    return info


environment_info = collect_environment_info()

with open(os.path.join(output_dir, 'environment_info.json'), 'w', encoding='utf-8') as f:
    json.dump(environment_info, f, ensure_ascii=False, indent=2)

with open(os.path.join(output_dir, 'environment_info.txt'), 'w', encoding='utf-8') as f:
    f.write("=" * 60 + "\n")
    f.write("實驗環境資訊 (論文表 4-1)\n")
    f.write("=" * 60 + "\n")
    f.write(f"OS / Platform     : {environment_info['os_platform']}\n")
    f.write(f"Python Version    : {environment_info['python_version']}\n")
    f.write(f"TensorFlow Version: {environment_info['tensorflow_version']}\n")
    f.write(f"Keras Version     : {environment_info['keras_version']}\n")
    f.write(f"GPU Name          : {environment_info['gpu_name']}\n")
    f.write(f"GPU Memory        : {environment_info['gpu_memory']}\n")
    f.write(f"Driver Version    : {environment_info['driver_version']}\n")
    f.write(f"CUDA Version      : {environment_info['cuda_version']}\n")
    f.write(f"cuDNN Version     : {environment_info['cudnn_version']}\n")

print(f"\n實驗環境資訊已保存: environment_info.txt / environment_info.json")


# ============================================================================
# experiment_config.json - 主要訓練設定（提升可重現性）
# ============================================================================
experiment_config = {
    ...
    'kfold_validation_augment': False,
    'test_augment_for_main_eval': False,
    'augmented_test_for_robustness_only': True,
    'kfold_pool': 'train + val',
    'tsne_data_source': 'unaugmented fold validation set',
    'checkpoint_monitor': 'val_accuracy on unaugmented fold validation set',
}
...
print(f"實驗設定已保存: experiment_config.json")
...
all_val_preds = []
all_val_labels_list = []
fold_class_dist_rows = []   # fold 類別分布（train / validation）
...
for fold, (train_idx, val_idx) in enumerate(skf.split(all_paths, all_labels)):
    ...
    # 建立 Dataset
    # [修正] 訓練集維持 augment=True；驗證集改為未擴增驗證折，
    #        使 ModelCheckpoint monitor='val_accuracy' 代表「未擴增驗證折」上的 accuracy，
    #        與論文報告的 fold accuracy 一致。
    train_ds = create_dataset(fold_train_paths, fold_train_labels, augment=True, shuffle=True)
    val_ds = create_dataset(fold_val_paths, fold_val_labels, augment=False, shuffle=False)

    # 計算 steps
    augmented_train_size = len(fold_train_paths) * AUGMENT_FACTOR
    # [修正] batch() 未設 drop_remainder=True，實際 step 數應以 ceil 計算
    ...
    # [新增] macro / weighted 平均指標
    macro_p, macro_r, macro_f1, _ = precision_recall_fscore_support(
        fold_val_labels, pred_labels, average='macro', zero_division=0)
    weighted_p, weighted_r, weighted_f1, _ = precision_recall_fscore_support(
        fold_val_labels, pred_labels, average='weighted', zero_division=0)

    # [新增] best_epoch：由 val_accuracy 最大值取得（1-based）
    ...
    # [新增] 每 fold 完整分類報告 -> classification_report_foldX.csv
    report_dict = pd.DataFrame(precision_recall_fscore_support(
        fold_val_labels, pred_labels, labels=list(range(num_classes)), zero_division=0
    ), index=['precision', 'recall', 'f1-score', 'support'], columns=class_names).T
    report_rows = []
    for cls_name in class_names:
        report_rows.append({
            'class': cls_name,
            'precision': report_dict.loc[cls_name, 'precision'],
            'recall': report_dict.loc[cls_name, 'recall'],
            'f1-score': report_dict.loc[cls_name, 'f1-score'],
            'support': int(report_dict.loc[cls_name, 'support']),
        })
    total_support = int(report_dict['support'].sum())
    report_rows.append({'class': 'macro avg', 'precision': macro_p, 'recall': macro_r,
                        'f1-score': macro_f1, 'support': total_support})
    report_rows.append({'class': 'weighted avg', 'precision': weighted_p, 'recall': weighted_r,
                        'f1-score': weighted_f1, 'support': total_support})
    pd.DataFrame(report_rows).to_csv(
        os.path.join(fold_dir, f'classification_report_fold{fold+1}.csv'),
        index=False, encoding='utf-8-sig'
    )

    # [新增] fold 類別分布（train / validation）
    for split_name, split_labels in [('train', fold_train_labels), ('validation', fold_val_labels)]:
        counts = np.bincount(split_labels, minlength=num_classes)
        row = {'fold': fold + 1, 'split': split_name}
        for ci, cls_name in enumerate(class_names):
            row[cls_name] = int(counts[ci])
        row['total'] = int(counts.sum())
        fold_class_dist_rows.append(row)
    ...
    # [新增] 另存混淆矩陣 CSV（未擴增驗證折）
    pd.DataFrame(cm_fold, index=class_names, columns=class_names).to_csv(
        os.path.join(fold_dir, f'confusion_matrix_fold{fold+1}.csv'), encoding='utf-8-sig'
    )

    # ========================================================================
    # 生成 t-SNE 視覺化（針對此 fold 的模型）
    # ========================================================================
    # [修正] t-SNE 改用該 fold 的未擴增驗證折資料（不再使用固定測試集）
    generate_tsne_visualization(
        best_model, val_ds_eval, fold_val_labels, fold_dir, len(fold_val_labels),
...
std_acc = np.std(accuracies, ddof=0)  # 統一使用 ddof=0
...
for r in fold_results:
    summary_rows.append({
        ...
        'train_time_seconds': r['duration'],
        'train_time_hms': str(timedelta(seconds=int(r['duration']))),
    })
pd.DataFrame(summary_rows).to_csv(
    os.path.join(output_dir, 'kfold_summary.csv'), index=False, encoding='utf-8-sig'
)
print(f"K-Fold 摘要已保存: {os.path.join(output_dir, 'kfold_summary.csv')}")

# ============================================================================
# [新增] fold_class_distribution.csv
# ============================================================================
pd.DataFrame(
    fold_class_dist_rows,
    columns=['fold', 'split'] + class_names + ['total']
).to_csv(
    os.path.join(output_dir, 'fold_class_distribution.csv'), index=False, encoding='utf-8-sig'
)
print(f"Fold 類別分布已保存: {os.path.join(output_dir, 'fold_class_distribution.csv')}")

# ============================================================================
# [新增] 整體 Out-of-Fold (OOF) 結果
# ============================================================================
oof_labels = np.array(all_val_labels_list)
oof_preds = np.array(all_val_preds)

# OOF 分類報告 CSV
oof_prfs = precision_recall_fscore_support(
    oof_labels, oof_preds, labels=list(range(num_classes)), zero_division=0
)
oof_macro_p, oof_macro_r, oof_macro_f1, _ = precision_recall_fscore_support(
    oof_labels, oof_preds, average='macro', zero_division=0)
oof_weighted_p, oof_weighted_r, oof_weighted_f1, _ = precision_recall_fscore_support(
    oof_labels, oof_preds, average='weighted', zero_division=0)
oof_acc = float(np.sum(oof_preds == oof_labels) / len(oof_labels))
oof_total_support = int(oof_prfs[3].sum())

oof_report_rows = []
for ci, cls_name in enumerate(class_names):
    oof_report_rows.append({
        'class': cls_name,
        'precision': oof_prfs[0][ci],
        'recall': oof_prfs[1][ci],
        'f1-score': oof_prfs[2][ci],
        'support': int(oof_prfs[3][ci]),
    })
oof_report_rows.append({'class': 'macro avg', 'precision': oof_macro_p, 'recall': oof_macro_r,
                        'f1-score': oof_macro_f1, 'support': oof_total_support})
oof_report_rows.append({'class': 'weighted avg', 'precision': oof_weighted_p, 'recall': oof_weighted_r,
                        'f1-score': oof_weighted_f1, 'support': oof_total_support})
pd.DataFrame(oof_report_rows).to_csv(
    os.path.join(output_dir, 'overall_oof_classification_report.csv'), index=False, encoding='utf-8-sig'
)

# OOF 混淆矩陣 CSV
pd.DataFrame(overall_cm, index=class_names, columns=class_names).to_csv(
    os.path.join(output_dir, 'overall_oof_confusion_matrix.csv'), encoding='utf-8-sig'
)

# OOF 指標 JSON
oof_metrics = {
    'accuracy': oof_acc,
    'macro_precision': float(oof_macro_p),
    'macro_recall': float(oof_macro_r),
    'macro_f1': float(oof_macro_f1),
    'weighted_f1': float(oof_weighted_f1),
}
with open(os.path.join(output_dir, 'overall_oof_metrics.json'), 'w', encoding='utf-8') as f:
    json.dump(oof_metrics, f, ensure_ascii=False, indent=2)
print(f"整體 OOF 結果已保存: overall_oof_classification_report.csv / "
      f"overall_oof_confusion_matrix.csv / overall_oof_metrics.json")
...
# ============================================================================
# [新增] 五個 Fold 模型於固定測試集上的結果（主要測試集結果）
# ============================================================================
print(f"\n{'='*80}")
print("測試集最終評估 - 五個 Fold 模型分別評估固定測試集")
print("="*80)

test_metrics_rows = []
for r in fold_results:
    fold_no = r['fold']
    fm_path = os.path.join(output_dir, f'fold_{fold_no}', f'best_model_fold{fold_no}.keras')
    print(f"\n評估 Fold {fold_no} 模型 (Val Acc: {r['accuracy']:.4f})...")
    fm = models.load_model(fm_path)

    # 原始測試集
    fm_pred = np.argmax(fm.predict(test_ds, verbose=0), axis=1)
    fm_test_acc_orig = float(np.sum(fm_pred == test_labels) / len(test_labels))
    _, _, fm_macro_f1_orig, _ = precision_recall_fscore_support(
        test_labels, fm_pred, average='macro', zero_division=0)

    # 擴增測試集
    fm_pred_aug = np.argmax(fm.predict(test_ds_augmented, verbose=0), axis=1)
    fm_test_acc_aug = float(np.sum(fm_pred_aug == test_labels_augmented) / len(test_labels_augmented))
    _, _, fm_macro_f1_aug, _ = precision_recall_fscore_support(
        test_labels_augmented, fm_pred_aug, average='macro', zero_division=0)

    print(f"  原始測試集: Acc={fm_test_acc_orig:.4f}, Macro-F1={fm_macro_f1_orig:.4f}")
    print(f"  擴增測試集: Acc={fm_test_acc_aug:.4f}, Macro-F1={fm_macro_f1_aug:.4f}")

    test_metrics_rows.append({
        'fold': fold_no,
        'val_accuracy': r['accuracy'],
        'test_acc_original': fm_test_acc_orig,
        'test_macro_f1_original': float(fm_macro_f1_orig),
        'test_acc_augmented': fm_test_acc_aug,
        'test_macro_f1_augmented': float(fm_macro_f1_aug),
    })
    del fm
    keras.backend.clear_session()
...
test_metrics_df.to_csv(
    os.path.join(output_dir, 'test_metrics_by_fold.csv'), index=False, encoding='utf-8-sig'
)
print(f"\n各 Fold 測試集結果已保存: {os.path.join(output_dir, 'test_metrics_by_fold.csv')}")

# 五個 fold 模型在測試集上的平均值與標準差
# [修正] 統一使用 np.std(values, ddof=0)，與 K-Fold accuracy 的 std 算法一致
test_fold_stats = {
    'test_acc_original': (np.mean(test_metrics_df['test_acc_original'].values), np.std(test_metrics_df['test_acc_original'].values, ddof=0)),
    'test_macro_f1_original': (np.mean(test_metrics_df['test_macro_f1_original'].values), np.std(test_metrics_df['test_macro_f1_original'].values, ddof=0)),
    'test_acc_augmented': (np.mean(test_metrics_df['test_acc_augmented'].values), np.std(test_metrics_df['test_acc_augmented'].values, ddof=0)),
    'test_macro_f1_augmented': (np.mean(test_metrics_df['test_macro_f1_augmented'].values), np.std(test_metrics_df['test_macro_f1_augmented'].values, ddof=0)),
}
print(f"\n五個 Fold 模型測試集平均 ± 標準差:")
print(f"  原始測試集 Acc     : {test_fold_stats['test_acc_original'][0]:.4f} ± {test_fold_stats['test_acc_original'][1]:.4f}")
print(f"  原始測試集 Macro-F1: {test_fold_stats['test_macro_f1_original'][0]:.4f} ± {test_fold_stats['test_macro_f1_original'][1]:.4f}")
print(f"  擴增測試集 Acc     : {test_fold_stats['test_acc_augmented'][0]:.4f} ± {test_fold_stats['test_acc_augmented'][1]:.4f}")
print(f"  擴增測試集 Macro-F1: {test_fold_stats['test_macro_f1_augmented'][0]:.4f} ± {test_fold_stats['test_macro_f1_augmented'][1]:.4f}")

# ============================================================================
# 測試集評估（補充：使用最佳 fold 模型，僅為 best-fold model 之補充結果）
# ============================================================================
print(f"\n{'='*80}")
print("測試集評估（補充：best-fold 單一模型）")
print("="*80)
...
print("（註：此為 best-fold 單一模型之補充結果，並非唯一測試集結果）")
...
with open(report_path, 'w', encoding='utf-8') as f:
    ...
    f.write("=" * 40 + "\n")
    f.write("K-Fold 評估資料來源:\n")
    f.write("=" * 40 + "\n")
    f.write("  - 訓練折: 使用擴增資料 (augment=True)\n")
    f.write("  - 驗證折: 使用未擴增的原始驗證折 (augment=False)\n")
    f.write("  - ModelCheckpoint: 監看未擴增驗證折的 val_accuracy\n")
    f.write("  - 混淆矩陣 / 分類報告: 使用未擴增驗證折\n")
    f.write("  - t-SNE 視覺化: 使用未擴增驗證折\n")
    f.write("  - 測試集: 全程保留，不參與 K-Fold (僅最終評估)\n\n")
    ...
    if USE_MULTICHANNEL:
        f.write(f"    通道 1: 翻轉後二值化影像\n")
        f.write(f"    通道 2: x 方向 Sobel 梯度響應 |G_x|\n")
        f.write(f"    通道 3: y 方向 Sobel 梯度響應 |G_y|\n\n")
    ...
    f.write("=" * 40 + "\n")
    f.write("各 Fold 結果（未擴增驗證折）:\n")
    f.write("=" * 40 + "\n")
    for r in fold_results:
        f.write(f"  Fold {r['fold']}: Acc={r['accuracy']:.4f}, Macro-F1={r['macro_f1']:.4f}, "
                f"Weighted-F1={r['weighted_f1']:.4f}, Best-Epoch={r['best_epoch']} "
                f"(訓練時間: {str(timedelta(seconds=int(r['duration'])))})\n")

    f.write(f"\n{N_FOLDS}-Fold 平均準確率: {mean_acc:.4f} ± {std_acc:.4f}\n\n")

    f.write("=" * 40 + "\n")
    f.write("整體 Out-of-Fold (OOF) 結果:\n")
    f.write("=" * 40 + "\n")
    f.write(f"  Accuracy       : {oof_acc:.4f}\n")
    f.write(f"  Macro Precision: {oof_macro_p:.4f}\n")
    f.write(f"  Macro Recall   : {oof_macro_r:.4f}\n")
    f.write(f"  Macro F1       : {oof_macro_f1:.4f}\n")
    f.write(f"  Weighted F1    : {oof_weighted_f1:.4f}\n\n")

    f.write("=" * 40 + "\n")
    f.write("固定測試集結果 - 五個 Fold 模型（主要結果）:\n")
    f.write("=" * 40 + "\n")
    for row in test_metrics_rows:
        f.write(f"  Fold {row['fold']}: 原始 Acc={row['test_acc_original']:.4f} "
                f"(Macro-F1={row['test_macro_f1_original']:.4f}), "
                f"擴增 Acc={row['test_acc_augmented']:.4f} "
                f"(Macro-F1={row['test_macro_f1_augmented']:.4f})\n")
    f.write(f"\n  五個 Fold 模型平均 ± 標準差:\n")
    f.write(f"    原始測試集 Acc     : {test_fold_stats['test_acc_original'][0]:.4f} ± {test_fold_stats['test_acc_original'][1]:.4f}\n")
    f.write(f"    原始測試集 Macro-F1: {test_fold_stats['test_macro_f1_original'][0]:.4f} ± {test_fold_stats['test_macro_f1_original'][1]:.4f}\n")
    f.write(f"    擴增測試集 Acc     : {test_fold_stats['test_acc_augmented'][0]:.4f} ± {test_fold_stats['test_acc_augmented'][1]:.4f}\n")
    f.write(f"    擴增測試集 Macro-F1: {test_fold_stats['test_macro_f1_augmented'][0]:.4f} ± {test_fold_stats['test_macro_f1_augmented'][1]:.4f}\n\n")

    f.write("=" * 40 + "\n")
    f.write(f"測試集結果（補充：best-fold 單一模型，Fold {best_fold['fold']}）:\n")
    f.write("=" * 40 + "\n")
    f.write("  註：以下僅為 best-fold 單一模型之補充結果，並非唯一測試集結果。\n")
    ...
    f.write(f"各類別 F1 分數（原始測試集, best-fold model）:\n")
    for i, cls_name in enumerate(class_names):
        f.write(f"  {cls_name} ({chinese_labels[i]}): {f1_scores[i]:.4f} (Support: {support[i]})\n")

    f.write(f"\n各類別 F1 分數（擴增測試集, best-fold model）:\n")
    for i, cls_name in enumerate(class_names):
...
print(f"  - classification_report_foldX.csv (完整分類指標)")
print(f"  - tsne_global_avg_pool.png (t-SNE GAP層, 驗證折)")
print(f"  - tsne_head_fc1.png (t-SNE FC1層, 驗證折)")
print(f"  - tsne_head_fc2.png (t-SNE FC2層, 驗證折)")
print(f"  - tsne_comparison.png (t-SNE 比較圖, 驗證折)")
print(f"\n根目錄論文用輸出:")
print(f"  - kfold_summary.csv (各 fold 摘要)")
print(f"  - fold_class_distribution.csv (各 fold 類別分布)")
print(f"  - overall_oof_classification_report.csv / overall_oof_confusion_matrix.csv / overall_oof_metrics.json")
print(f"  - test_metrics_by_fold.csv (五個 fold 模型測試集結果)")
print(f"  - environment_info.txt / environment_info.json (實驗環境, 表 4-1)")
print(f"  - experiment_config.json (訓練設定)")
print("="*80)